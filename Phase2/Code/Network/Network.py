...
def LossFn(network_output, labels):
    ...
    # loss = torch.mean(0.5*0.0.03125 (labels - network_output)**2)
    ...
    ###############################################
    loss  = criterion(labels, network_output)
    return loss
...
class supervised_Homography_net(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Sequential(nn.Conv2d(6,64,kernel_size = 3), nn.BatchNorm2d(64), nn.ReLU())
