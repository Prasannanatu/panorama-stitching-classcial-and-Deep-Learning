...
from Network.Network import HomographyModel, supervised_Homography_net
import cv2
import sys
import os
import numpy as np
...
def GenerateBatch(BasePath, DirNamesTrain, TrainCoordinates, ImageSize, MiniBatchSize, Process="Train"):
    ...
    while ImageNum < MiniBatchSize:
        ...
        patched_image = patched_image.reshape(128, 128, 3)
        ...
        warped_image = warped_image.reshape(128, 128, 3)
        norm1 = np.zeros((128,128,3))
        warped_image = cv2.normalize(warped_image,  norm1, 0, 1, cv2.NORM_MINMAX)
        ...
        stacked_image = stacked_image.view(6, 128, 128)
        ...
        # Patch_a.append(torch.tensor(point_patch_1))
        # image_1.append(torch.tensor(patched_image))
        # print("ImageNum",ImageNum, type(ImageNum))
        # print("MiniBatchSize",MiniBatchSize, type(MiniBatchSize))
    return torch.stack(I1Batch).to(device), torch.stack(CoordinatesBatch).to(device)#,torch.stack(Patch_a).to(device),image_1(torch.stack())
...
def TrainOperation(
    ...
    for Epochs in tqdm(range(StartEpoch, NumEpochs)):
        NumIterationsPerEpoch = int(NumTrainSamples*3 / MiniBatchSize / DivTrain)
        ...
        SaveName = CheckPointPath + str(Epochs) + "pmodel.ckpt"
        torch.save(
            {
                "epoch": Epochs,
                "model_state_dict": model.state_dict(),
...
def main():
    ...
    Parser.add_argument(
        "--CheckPointPath",
        default="../Checkpoints/sup4/",
        help="Path to save Checkpoints, Default: ../Checkpoints/sup4/",
    )
    ...
    Parser.add_argument(
        "--LogsPath",
        default="Logs/sup4/",
        help="Path to save Logs for Tensorboard, Default=Logs/sup4/",
    )
