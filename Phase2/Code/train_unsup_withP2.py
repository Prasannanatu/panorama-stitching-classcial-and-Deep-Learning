...
import torchvision
from Network.Network_Unsup_withP2 import LossFn
from torch.utils.tensorboard import SummaryWriter
from torchvision import datasets, transforms
from torch.optim import AdamW
from torch.optim import SGD
from Network.Network_Unsup_withP2 import HomographyModel
import cv2
import sys
import os
import numpy as np
...
def GenerateBatch(BasePath, DirNamesTrain, TrainCoordinates, ImageSize, MiniBatchSize, Process="Train"):
    ...
    while ImageNum < MiniBatchSize:
        ...
        image_2.append(torch.tensor(warped_image,dtype=torch.double))
        # print("ImageNum",ImageNum, type(ImageNum))
        # print("MiniBatchSize",MiniBatchSize, type(MiniBatchSize))
    return torch.stack(I1Batch).to(device), torch.stack(CoordinatesBatch).to(device),torch.stack(Patch_a).to(device),torch.stack(Patch_b).to(device),torch.stack(image_1).to(device),torch.stack(image_2).to(device)
...
def TrainOperation(
    ...
    for Epochs in tqdm(range(StartEpoch, NumEpochs)):
        ...
        for PerEpochCounter in tqdm(range(NumIterationsPerEpoch)):
            I1Batch,CoordinatesBatch,patch_a,patch_b,image_1,image_2 = GenerateBatch(
            ...
            model.train()
            # Predict output with forward pass
            PredicatedCoordinatesBatch = model(I1Batch,CoordinatesBatch,patch_a,patch_b,image_1,image_2)
            ...
            with torch.no_grad():
                ...
                validation_batch, validation_labels,patch_a,patch_b,image_1,image_2 = GenerateBatch(
                    BasePath, Validation_set, TrainCoordinates, MiniBatchSize, "Validation")

            result = model.validation_step(validation_batch, validation_labels,patch_a,patch_b,image_1,image_2)
        ...
        SaveName = CheckPointPath + str(Epochs) + "unsup44model.ckpt"
        torch.save(
            {
                "epoch": Epochs,
                "model_state_dict": model.state_dict(),
